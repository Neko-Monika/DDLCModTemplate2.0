    ...
    if not renpy.android:
        for archive in ["audio", "fonts", "images"]:
            if archive not in config.archives:
                raise Exception("Файлы архивов DDLC не были найдены в папке /game. Проверьте правильность установки и повторите попытку.")

        if renpy.windows:
            try:
                onedrive_path = os.environ["OneDrive"]
                if onedrive_path in config.basedir:
                    raise Exception("Модификации к DDLC или проекты оных не могут быть запущены из папки облака. Переместите её папку в другую директорию и повторите попытку.")
            except: pass
