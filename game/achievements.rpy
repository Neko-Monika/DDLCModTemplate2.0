...
# Этот файл не является частью DDLC. Данный файл содержит код меню достижений
# и уведомления, отображающего прогресс прохождения модификации.
...
init python:
    ...
    # В этом разделе объявляются достижения. См. класс «Achievements» для объявления своих достижений.
...
## al.maxCount - Эта содержит число, которое необходимо набрать для получения
##               достижения.
...
Продолжайте своё прохождение «[config.name]», чтобы открыть все доступные достижения.""", ok_action=Hide("dialog"))
        ...
        if config.developer:
            textbutton "Тест уведомления":
                style "return_button"
                xpos 0.8 ypos 1.1
                action Show("achievement_notify", startup)
